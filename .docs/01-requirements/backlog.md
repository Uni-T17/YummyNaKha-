...
| # | Item | Traces to |
|---|------|-----------|
| B1 | Account and sign-in so a food profile persists — **mechanism `TBD`** | F1, P3 |
...
| B20 | Third-party OCR/AI transfer disclosed, with consent obtained before the first send | LR2 (PDPA) |
| B21 | Outbound payload minimisation — menu text plus the profile flags the recommendation needs, no account email, real name, or other identifier | LR3 (PDPA) |
| B22 | Purpose limit on profile and menu data — no advertising, resale, or profiling without a separate opt-in | LR4 (PDPA) |
| B23 | Deletion handling; a menu image is processed then discarded by default, with any temporary retention justified, time-bounded, and disclosed | LR5 (PDPA) |
| B24 | Login stores a verification token only, never a provider credential | LR6 (PDPA) |
...
| B26 | No over-retention: profiles, menu images, history, and selected dishes are **not** kept 90 days merely because of §26 | LR8 (CCA §26) |
...
| # | Item | Traces to |
|---|------|-----------|
| B32 | Scan history — reopen a previously scanned menu | F20 |
...
| OCR/AI provider and cost-model decision | NFR6, and the provider-specific wording of B20, B21 |
| First usability test | NFR8 |
| Team decision on the authentication mechanism | B1, and the exact shape of B24 |
