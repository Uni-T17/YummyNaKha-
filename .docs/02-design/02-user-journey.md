...
|---|--------|--------|-------------|--------|---------------|
| J1 | Sign-up & Terms | Creates an account; accepts the Terms, privacy notice, and AI & Accuracy Disclaimer | Stores each acceptance with user ID, document version, timestamp, and action, retrievable later. Stores a verification token only | F1, F15, LR6, LR9 | — (trust foundation) |
...
| J5 | Transfer consent *(first scan only)* | Reads what is sent and consents — separately from the Terms already accepted at J1 | Discloses that an external OCR/AI provider receives the data; sends menu text and the needed profile flags only, never account email or real name; stores the consent record | F15, LR2, LR3, LR9 | **P4** |
| J6 | Processing | Waits | Extracts each dish name and price → translates Thai → English → infers likely ingredients → checks every dish against the profile. Shows progress; completes within the latency target | F6, F7, F8, F9, NFR2, NFR3, NFR5 | **P1** |
...
| J10 | Ongoing control | Later: edits the profile, withdraws a consent, or deletes the account | Applies the edit; stores a withdrawal the same retrievable way as the consent; on deletion removes profile, images, and results — traffic logs excepted where CCA §26 applies | F4, F16, F17, LR5, LR7, LR9 | — (data rights) |
...
|---|---|---|---|
| A1 | The photo is unreadable — blur, glare, angle, handwriting | Plain-language message saying what to do next, never a raw error | F18, FE8 AC1 |
...
| A10 | D deletes the account | Profile, uploaded images, and results are removed; traffic logs are retained where CCA §26 applies, separately from content | F17, LR5, LR7, LR8 |
...
- **P5** (wanting a direct photo-based route) → J4, J8
...
Scan history (F20) and rating a recommendation (F21) are Could and outside the
core workflow. Live camera scanning (F22), voice output (F23), other language
pairs (F24), and restaurant-side data (F25) are Won't.
