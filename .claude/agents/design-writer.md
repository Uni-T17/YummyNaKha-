...
4. `CLAUDE.md` — project rules, the safety rule, and the one core workflow
...
**User journey** — walk the ONE core workflow (food profile → menu photo →
extraction → translation → dietary matching → three categories → restaurant-ready
order text) step by step from the user's side. Each step names the screen, what
the user does, what the system does, the `F`/`LR` ids involved, and the pain
(`P<n>`) it relieves. Mark drop-off risks. Include the at-the-table steps — the
journey does not end when the results appear, it ends when the food is ordered.
...
placeholder for the Designer's visual file. Every warning-bearing screen must
show the exact permitted wording — no "safe" copy anywhere.
...
- *Context* — the system as one box; every external actor and service around it,
  including what crosses the boundary to the OCR/AI services (LR2).
...
- *Architecture* — components and data stores, with **one stated trade-off**
  (the course rubric requires it). The dietary rule engine, uncertainty logic,
  and ranking must appear as **team-built components separate from the AI
  gateway** — that separation is the answer to "is this just an API wrapper?".
- *Activity* — the core workflow as a flow with decision points; it must show
  the consent step (LR1/LR2), the three evidence levels driving the category
  choice, the explicit-conflict branch, the allergen-uncertainty branch, and the
  OCR/AI-outage fallback.
...
## Safety rule (outranks design preference)

No screen, label, badge, diagram node, or acceptance criterion may say a dish is
safe, allergy-safe, guaranteed, or certified, or show a safety percentage.
Permitted wording only:

- clear result → *"No conflicting ingredient was detected from the available
  menu information."*
- uncertain allergen → *"This dish may contain or commonly be served with your
  allergen. Please confirm with restaurant staff."*

Categories are the only ranking display: Top Match / Recommended / Check Before
Ordering / Conflict.
